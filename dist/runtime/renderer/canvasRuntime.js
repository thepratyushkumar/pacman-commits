...
function getCellColorSet(palette, activeContribution, powerCell, contributionStrength, maxContributionStrength) {
...
function createBoardLayout(cellSize) {
...
function directionalOffset(direction, magnitude) {
    switch (direction) {
        case "up":
...
function drawGhost(context, ghost, color, frightened, layout, palette) {
    ...
    context.beginPath();
    context.arc(-radius * 0.26 + pupilOffset.col, -radius * 0.15 + pupilOffset.row, radius * 0.09, 0, Math.PI * 2);
    context.arc(radius * 0.3 + pupilOffset.col, -radius * 0.15 + pupilOffset.row, radius * 0.09, 0, Math.PI * 2);
    context.fill();
    context.restore();
}
function drawPacman(context, state, layout, color) {
    ...
    const gradient = context.createRadialGradient(x - radius * 0.28, y - radius * 0.36, radius * 0.08, x, y, radius);
...
function drawBoard(context, state, bootstrap, canvas, layout, mazeSegments) {
    ...
    const maxContributionStrength = Math.max(1, ...state.map.contributionStrength.flat().map((value) => value ?? 0));
    ...
    roundedRect(context, panelX, panelY, panelWidth, panelHeight, 30);
    context.stroke();
    const innerGradient = context.createLinearGradient(innerPanelX, innerPanelY, innerPanelX, innerPanelY + innerPanelHeight);
    ...
    for (let row = 0; row < GRID_HEIGHT; row += 1) {
        for (let col = 0; col < GRID_WIDTH; col += 1) {
            ...
            const floorColors = getCellColorSet(palette, activeContribution, cell === 4, contributionStrength, maxContributionStrength);
            ...
            roundedRect(context, x + 1.2, y + 1.2, layout.square - 2.4, Math.max(5, layout.square * 0.28), 4);
            context.fill();
            if (cell !== 4) {
                continue;
            }
    ...
    const sweepGradient = context.createLinearGradient(innerPanelX + sweepOffset, innerPanelY, innerPanelX + sweepOffset + 180, innerPanelY + innerPanelHeight);
    ...
    state.ghosts.forEach((ghost, index) => {
        ...
        drawGhost(context, ghost, state.frightenedTurnsRemaining > 0
            ...
            : palette.ghosts[index % palette.ghosts.length], state.frightenedTurnsRemaining > 0, layout, palette);
    });
}
