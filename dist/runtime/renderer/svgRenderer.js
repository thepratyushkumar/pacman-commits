...
export function renderGameSvg(simulation, config) {
    ...
    const sourceLabel = simulation.source.source === "live"
    ...
    svg += `<desc id="desc">${escapeXml(`Pac-Man traverses a GitHub contribution board while ghosts chase through the graph. Final score ${simulation.finalState.score}.`)}</desc>`;
    ...
    svg += `<text x="${left}" y="${height - 18}" fill="${palette.monthLabel}" font-size="11" font-family="'Trebuchet MS', sans-serif">${escapeXml(`${sourceLabel} • score ${simulation.finalState.score} • ${simulation.frames.length} frames`)}</text>`;
    let lastMonth = "";
    simulation.frames[0].monthLabels.forEach((label, col) => {
        ...
        svg += `<text x="${x}" y="${top - 14}" text-anchor="middle" fill="${palette.monthLabel}" font-size="10" font-family="'Trebuchet MS', sans-serif">${escapeXml(label)}</text>`;
    });
    for (let row = 0; row < GRID_HEIGHT; row += 1) {
        for (let col = 0; col < GRID_WIDTH; col += 1) {
            ...
            const pelletFrames = simulation.frames.map((frame) => frame.grid[row][col] === 1 ? "1" : "0");
