...
export function renderGameSvg(simulation, config) {
    ...
    const mazePath = mazeSegments
        ...
        .join(" ");
    svg += `<path d="${mazePath}" fill="none" stroke="${withAlpha("#04070d", 0.7)}" stroke-width="${Math.max(5.2, gap + 3.1)}" stroke-linecap="round" stroke-linejoin="round"/>`;
    svg += `<path d="${mazePath}" fill="none" stroke="${withAlpha("#f8fbff", 0.94)}" stroke-width="${Math.max(2.2, gap + 0.6)}" stroke-linecap="round" stroke-linejoin="round" filter="url(#maze-glow)"/>`;
