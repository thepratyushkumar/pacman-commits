...
function createMatrix(factory) {
    return Array.from({ length: GRID_HEIGHT }, (_, row) => Array.from({ length: GRID_WIDTH }, (_, col) => factory(row, col)));
}
...
function normalizeColumns(columns) {
    const trimmed = columns.slice(-GRID_WIDTH).map((column) => Array.from({ length: GRID_HEIGHT }, (_, index) => {
    ...
    return [
        ...Array.from({ length: GRID_WIDTH - trimmed.length }, () => Array.from({ length: GRID_HEIGHT }, () => ({
            commitsCount: 0,
            level: "NONE",
            color: "",
        }))),
...
function normalizeMonthLabels(labels) {
    const trimmed = labels.slice(-GRID_WIDTH);
    return [...Array.from({ length: GRID_WIDTH - trimmed.length }, () => ""), ...trimmed];
}
function findNearestOpenCell(walls, preferred, predicate = () => true) {
    let best = null;
...
function carveCell(baseCells, walls, position) {
...
function carveRoom(baseCells, walls, center, radius) {
...
function carvePath(baseCells, walls, from, to) {
...
function collectConnectedComponents(walls) {
...
function connectComponents(baseCells, walls) {
...
function choosePowerPellets(baseCells, contributionStrength, blocked) {
    const corners = [
        { row: 0, col: 0 },
    ...
    for (const corner of corners) {
        ...
        for (let row = 0; row < GRID_HEIGHT; row += 1) {
            for (let col = 0; col < GRID_WIDTH; col += 1) {
                ...
                const score = contributionStrength[row][col] * 10 -
...
function ensureStarterPellets(baseCells, walls) {
    const pellets = [];
    const starterCells = [
        { row: 3, col: 2 },
        { row: 3, col: 3 },
...
export function generateGameMap(snapshot, _config) {
    const normalizedColumns = normalizeColumns(snapshot.columns);
    const monthLabels = normalizeMonthLabels(snapshot.monthLabels);
    const baseCells = createMatrix(() => 0);
    const contributionStrength = createMatrix(() => 0);
    const walls = createMatrix(() => true);
    for (let col = 0; col < GRID_WIDTH; col += 1) {
    ...
    const pacmanSpawn = findNearestOpenCell(walls, pacmanPreferred, (position) => position.col < 12) ??
        pacmanPreferred;
    const ghostSpawn = findNearestOpenCell(walls, ghostPreferred, (position) => position.col > 16 && position.col < 36) ??
        ghostPreferred;
    carveRoom(baseCells, walls, pacmanSpawn, 1);
    carveRoom(baseCells, walls, ghostSpawn, 1);
    const nearestPelletToSpawn = findNearestOpenCell(walls, pacmanSpawn, (position) => baseCells[position.row][position.col] === 1) ??
        ghostSpawn;
    const nearestPelletToHouse = findNearestOpenCell(walls, ghostSpawn, (position) => baseCells[position.row][position.col] === 1) ??
        pacmanSpawn;
