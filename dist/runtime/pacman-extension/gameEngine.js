import { bfsToGoal, chooseGhostStep, directionToVector, getNeighbors, manhattanDistance, } from "./ghostAI.js";
import { consumeCurrentCell, resolveGhostCollision, tickPowerupTimer } from "./powerups.js";
import { GHOST_IDS, GRID_HEIGHT, GRID_WIDTH, } from "./types.js";
...
function isWalkable(map, position) {
...
function buildGhostSpawnSlots(map) {
    const preferred = [
        map.ghostSpawn,
    ...
    return preferred.map((position) => isWalkable(map, position) ? position : map.ghostSpawn);
}
function buildRenderGrid(cells, pacman, ghosts) {
...
function determinePacmanDirection(state, config, requestedDirection) {
...
function chooseAutoplayDirection(state) {
    ...
    const shouldPrioritizePowerPellet = state.frightenedTurnsRemaining === 0 &&
        state.ghosts.some((ghost) => ghost.released && manhattanDistance(ghost, state.pacman) <= 6);
    const findPath = (preferPowerPellets, avoidDanger) => bfsToGoal(state, state.pacman, (position) => {
    ...
    const path = (shouldPrioritizePowerPellet && findPath(true, true)) ||
        findPath(false, true) ||
...
function advanceModeTimers(state) {
    if (state.frightenedTurnsRemaining > 0) {
...
function movePacman(state, config, requestedDirection) {
...
function moveGhosts(state, pacmanPreviousPosition) {
    for (const ghost of state.ghosts) {
        ...
        const crossed = ghostPrevious.row === state.pacman.row &&
...
function buildGhosts(map, ghostCount) {
    const corners = [
...
export function createInitialGameState(map, config) {
...
export function stepGame(previousState, config, requestedDirection) {
