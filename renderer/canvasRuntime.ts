...
import type { BrowserBootstrapData, Direction, GameConfig, GameState, Position } from "../pacman-extension/types.js";
...
function scaleCanvas(canvas: HTMLCanvasElement, width: number, height: number): CanvasRenderingContext2D {
    ...
    for (let col = 0; col < GRID_WIDTH; col += 1) {
      ...
      if (cell === 1 || cell === 4) {
        ...
        context.arc(
          x + square / 2,
          y + square / 2,
          cell === 4 ? Math.max(3.5, square * 0.2) : Math.max(2, square * 0.12),
          0,
          Math.PI * 2,
        );
        context.fill();
...
export function mountPacmanDemo(root: HTMLElement, bootstrap: BrowserBootstrapData): void {
  ...
  function updateLabels(): void {
    ...
    stats.textContent = `Score ${state.score} • Pellets ${state.pelletsRemaining} • Turn ${state.turn}/${bootstrap.config.maxTurns} • ${
      state.frightenedTurnsRemaining > 0 ? `Frightened ${state.frightenedTurnsRemaining}` : state.modePhase
    } • ${state.lastEvent}`;
  }
